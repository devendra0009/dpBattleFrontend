...
const Profile = ({
  ...
  // Add null checks for user and followers
  if (!user) {
    return null;
  }
  ...
  console.log(followers, currentUser?._id);
  const isFollowed = followers.some(
    (uid) => uid && uid.toString() === currentUser?._id?.toString()
  );
  ...
  const handleFollowClick = async () => {
    ...
    try {
      ...
      // Only update if the action was fulfilled and has the updated target user
      if (
        updatedData.type === "user/updateFollowerFollowing/fulfilled" &&
        updatedData.payload?.resData?.updatedTargetUser
      ) {
          ...
          >
            ...
            <UserDetails className="userDetails">
              <UserName currentTheme={currentTheme}>
                {user?.name?.toUpperCase() || "Unknown"} <MdVerified />
              </UserName>
              {/* <UserInfo> */}
              <UserMid>
...
const TabButton = styled.button`
  ...
  background-color: ${(props) =>
    props.isactive ? "lightgreen" : props.theme[props.currentTheme].bg};
